  ...
  app.get("/auth/callback", async (c) => {
    ...
    try {
      ...
      if (!email || !verified || !isAllowed(email)) {
        ...
        // Surface WHY on the login screen: the email used + what's allowed.
        const q = new URLSearchParams({ auth: "denied", e: email || "", d: ALLOWED_DOMAIN || "" });
        return c.redirect(`/?${q.toString()}`);
      }
      // eslint-disable-next-line no-console

...
function buildSystem(
  ...
  return [
    ...
    `- Source legitimacy: prefer authoritative sources — the company's own newsroom/investor`,
    `  relations, SEC/EDGAR, PR wires (BusinessWire, PR Newswire, GlobeNewswire), major news`,
    `  outlets (Reuters, Bloomberg, WSJ, CNBC, AP), and established retail trade press`,
    `  (Retail Dive, WWD, Chain Store Age, Sourcing Journal, Modern Retail, Footwear News).`,
    `  Do NOT fire a signal on a single low-authority source (content farm, SEO blog, unsourced`,
    `  aggregator, press-release reprint with no named outlet). When sources conflict, trust the`,
    `  most authoritative and most recent. Cite the most credible URL available for each signal.`,
